...
cc.Class({
    ...
    onLoad: function () {
        var size = cc.winSize;
        this.score_sprite.string = window.userData.score4.toString();
    ...
    update:function(){
        ...
        var k=15;
        if(delta > k && window.userData.score4>0){
    ...
    demoEnd:function(){
        ...
        sc.name = 'LV2';
        window.userData.name = sc.name;
        sc.setPosition(cc.p(size.width*5/9 - size.width/2, size.height*5/6 - size.height/2 ));
        this.node.addChild(sc);
        this.scroll_array.push(sc);
